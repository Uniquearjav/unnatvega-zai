...
/* ─────────────── Cover Placeholder ─────────────── */
function CoverPlaceholder({ blogId, isFeatured = false }: { blogId: string; isFeatured?: boolean }) {
  const index = parseInt(blogId, 10) % coverIcons.length;

  return (
    <div className="relative flex size-full items-center justify-center overflow-hidden bg-gradient-to-br from-primary/20 via-primary/8 to-transparent">
      {/* Decorative grid */}
...
/* ─────────────── Featured Post ─────────────── */
function FeaturedPost({ blog, index = 0 }: { blog: BlogPost; index?: number }) {
  ...
  return (
    ...
    >
      <Link href={`/blog/${blog.slug}`} className="group block">
        <div className="relative overflow-hidden rounded-2xl border border-border/40 bg-card/50 backdrop-blur-sm transition-all duration-500 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5">
          <div className="grid md:grid-cols-2">
            ...
            <div className="relative h-56 overflow-hidden sm:h-64 md:h-full md:min-h-[320px]">
              {blog.coverImage ? (
                ...
                <CoverPlaceholder blogId={blog.id} isFeatured />
              )}
              {/* Gradient overlay */}
...
/* ─────────────── Blog Card ─────────────── */
function BlogCard({ blog, index = 0 }: { blog: BlogPost; index?: number }) {
  const tags = blog.tags?.split(',').map((t) => t.trim()) || [];
  ...
  return (
    ...
    >
      <Link href={`/blog/${blog.slug}`} className="group block h-full">
        <div className="h-full overflow-hidden rounded-2xl border border-border/40 bg-card/50 backdrop-blur-sm transition-all duration-500 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5">
          {/* Cover */}
          <div className="relative h-44 overflow-hidden">
            {blog.coverImage ? (
              ...
              <CoverPlaceholder blogId={blog.id} />
            )}
            {/* Bottom gradient */}
          ...
          <div className="p-5">
            {/* Meta */}
            <div className="mb-2.5 flex items-center gap-2.5 text-[11px] text-muted-foreground">
            ...
            {/* Excerpt */}
            <p className="mb-4 line-clamp-2 text-xs leading-relaxed text-muted-foreground sm:text-sm">
              {blog.excerpt}
            </p>
          ...
          {loading ? (
            <div className="space-y-8">
              ...
              <div className="animate-pulse overflow-hidden rounded-2xl border border-border/40">
                <div className="grid md:grid-cols-2">
                  <div className="h-56 bg-muted/50 sm:h-64 md:h-full md:min-h-[320px]" />
                  <div className="flex flex-col justify-center p-6 sm:p-8">
                    <div className="mb-4 flex gap-2">
                      <div className="h-5 w-16 rounded-full bg-muted/70" />
                      <div className="h-5 w-20 rounded-full bg-muted/70" />
                    </div>
                    <div className="mb-3 h-7 w-full rounded bg-muted/70" />
                    <div className="mb-2 h-5 w-5/6 rounded bg-muted/50" />
                    <div className="mb-6 h-5 w-4/6 rounded bg-muted/50" />
            ...
            <>
              ...
              {featuredPost && !searchQuery && !selectedTag && (
                <div className="mb-8 sm:mb-10">
                  <FeaturedPost blog={featuredPost} index={0} />
                </div>
              )}

              {/* Section header */}
              {otherPosts.length > 0 && !searchQuery && !selectedTag && (
                <AnimatedSection className="mb-6">
                  <div className="flex items-center gap-3">
                    <div className="h-px flex-1 bg-border/40" />
                    <h2 className="text-sm font-medium uppercase tracking-wider text-muted-foreground">Latest Articles</h2>
                    <div className="h-px flex-1 bg-border/40" />
                  </div>
                </AnimatedSection>
              )}
